...
while running:
    ...
    # minimum bets
        ...
        if game_state.players[current_player].folded:
            ...
            o = calculations.determine_move(game_state.players[current_player].hand + game_state.river, len(game_state.river), game_state.get_call_amount(current_player))
            game_state.select_player_option(current_player, o[0], o[1])
            current_player += 1
        else:
            o = calculations.determine_move(game_state.players[current_player].hand + game_state.river, len(game_state.river), game_state.get_call_amount(current_player))
            game_state.select_player_option(current_player, o[0], o[1])
        ...
        tentative_bet_text.add(
            element.TextElement(f"${game_state.get_call_amount(0)}", 'fonts/CallingCode-Regular.ttf', 28, 800, 650).sprite)
